...
import { ICardscontainerDocument, ObjectIdType, IUser } from '../types'
import { cardscontainerDefault } from '../utils/card-and-container-default'

const newCardscontainer = async (associateUserId: ObjectIdType): Promise<ICardscontainerDocument> => {
  ...
  const createdCardscontainer = await repository.create(initialCardscontainerAndCard) as ICardscontainerDocument
...
const getCardscontainersByUser = async (userInfo: IUser) => {
  ...
  const repository = new CardscontainerRepository(CardscontainerModel)
  const userCardscontainers = await repository.find({ associateUserId: userId })

  return userCardscontainers

}

const getCardscontainerById = async (id: ObjectIdType): Promise<ICardscontainerDocument[]> => {
  const repository = new CardscontainerRepository(CardscontainerModel)
...
export { newCardscontainer, removeCardscontainer, getCardscontainersByUser, getCardscontainerById }
