...
import { ICardscontainerDocument, IUser, IUserDocument, ObjectIdType } from '../types'
import { newCardscontainer } from '../services/cards-container.service'

const getUserId = async (userInfo: IUser): Promise<IUserDocument[]> => {
  const { name, password } = userInfo
  const repository = new UserRepository(UserModel)
  const userDocument = await repository.find({ name: name, password: password })
